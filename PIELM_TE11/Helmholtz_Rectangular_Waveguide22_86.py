...
ax.scatter3D(X_g[:, 0], X_g[:, 1], c='green', label='Interior points')
ax.scatter3D(X_b[:, 0], X_b[:, 1], c='blue', label='Boundary points')
ax.scatter3D(X_i[:, 0], X_i[:, 1], c='red', label='Initial points')
...
# 绘制二维平面图
plt.scatter(X_g[:, 0], X_g[:, 1], c='green', label='Interior points')
plt.scatter(X_b[:, 0], X_b[:, 1], c='blue', label='Boundary points')
plt.scatter(X_i[:, 0], X_i[:, 1], c='red', label='Initial points')
...
plt.title('Sampling Points')  # 添加图标题

plt.show()  # 显示图形

plt.savefig('Helmholtz_PIELM_TE11_sampling_points.png', dpi = 500, bbox_inches='tight')
...
for j in range(Num_n):
    ...
    #dx = torch.autograd.grad(outputs=u, inputs=a, grad_outputs=torch.ones_like(u), create_graph=True)[0][:, 0].view(-1,
      #                                                                                                              1)
    #dy = torch.autograd.grad(outputs=u, inputs=a, grad_outputs=torch.ones_like(u), create_graph=True)[0][:, 1].view(-1,
           #                                                                                                         1)
    #dxx = torch.autograd.grad(outputs=dx, inputs=a, grad_outputs=torch.ones_like(dx), create_graph=True)[0][:, 0].view(
       # -1, 1)

    #dyy = torch.autograd.grad(outputs=dy, inputs=a, grad_outputs=torch.ones_like(dy), create_graph=True)[0][:, 1].view(
      #  -1, 1)
...
for j in range(Num_n):
    ...
    A_b_lr[:,j:j+1] = bdx *sigma_std/X_std[0]
...
for j in range(Num_n):
    ...
    A_b_du[:, j:j + 1] = bdy *sigma_std/X_std[1]
...
#print('ppppp=',p)
#############到这里了
data = genfromtxt('exact_mm100.txt', delimiter=',') #用于读取名为val1.txt的文本文件中的数据
...
#################
np.savetxt('exact_u.txt',data[:,2], delimiter = ',')
np.savetxt('PIELM.txt',sigma.detach().numpy(), delimiter = ',')

# 将pinn100.txt的前两列和sigma的值合并成一个新的数组
result_data = np.column_stack((data[:, 0], data[:, 1], sigma.detach().numpy()))

# 将结果保存到'PIELM_2.txt'
np.savetxt('PIELM_2.txt', result_data, delimiter=',')
...
# Assuming `data` and `sigma` are already defined
...
# Extract X and Y coordinates from the first two columns of the original data
X = data[:, 0]
Y = data[:, 1]

# Extract the values to be represented by color
Z_data = data[:, 2]
Z_sigma = sigma.detach().numpy()

# Create a 2D grid for X and Y using every 100th point
X_grid = X[::100]
Y_grid = Y[::100]
X_grid, Y_grid = np.meshgrid(X_grid, Y_grid)

# Interpolate Z_data and Z_sigma on the grid
Z_data_grid = griddata((X, Y), Z_data, (X_grid, Y_grid), method='linear')
Z_sigma_grid = griddata((X, Y), Z_sigma, (X_grid, Y_grid), method='linear')

# Create subplots with 1 row and 2 columns
fig, axes = plt.subplots(1, 2, figsize=(12, 5))

# Plot the first heatmap (based on data[:,2])
im1 = axes[0].imshow(Z_data_grid, extent=(X.min(), X.max(), Y.min(), Y.max()), origin='lower', aspect='auto', cmap='viridis')
axes[0].set_title('Heatmap of Data[:,2]')
axes[0].set_xlabel('X-axis')
axes[0].set_ylabel('Y-axis')
fig.colorbar(im1, ax=axes[0])

# Plot the second heatmap (based on sigma)
im2 = axes[1].imshow(Z_sigma_grid, extent=(X.min(), X.max(), Y.min(), Y.max()), origin='lower', aspect='auto', cmap='viridis')
axes[1].set_title('Heatmap of Sigma')
...
# 误差图
plt.subplot(1, 3, 3)
pcm = plt.pcolor(X, Y, Z_error, cmap='jet', vmin=0, vmax=0.05)
#pcm = plt.pcolor(X, Y, Z_error, cmap='jet', vmin=0, vmax=1)
plt.xlabel(r'$x(mm)$', fontsize=18)
plt.ylabel(r'$y(mm)$', fontsize=18)
...
plt.savefig('Helmholtz_rec_PIELM_TE11.tif', dpi=1000, bbox_inches='tight')
plt.show()
